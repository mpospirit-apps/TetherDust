...
import { getDocGenStatus, type DocGenStatus } from "../../api/docs";
...
export function SourceSelect({
  ...
  if (empty) {
    return <p className="text-sec text-sm">No databases, docs, or codebases configured yet.</p>;
  }
  return (
    <div>
      <CheckColumn
...
export function StatusPanel({ status }: { status: DocGenStatus }) {
  if (status.status === "running") {
    return (
      <div className="doc-loading-state">
        ...
        {status.agent_output && (
          <p className="doc-loading-elapsed">{status.agent_output.slice(0, 200)}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className={isPartial ? "flash flash-warning" : "flash flash-success"}>
        {isPartial ? "Completed with warnings." : "Documentation generated successfully."}
      </div>
      {(status.warnings ?? []).map((w, i) => (
        <div key={i} className="flash flash-warning" style={{ marginTop: "var(--sm)" }}>
          {w}
        </div>
      ))}
      <div className="doc-result-grid" style={{ marginTop: "var(--md)" }}>
        <div className="doc-result-summary">
          ...
          <p>{fmtBytes(status.is_library ? status.total_size : status.file_size)}</p>
        </div>
          ...
          {status.is_library ? (
              ...
              <ul className="text-sm">
                {(status.files ?? []).map((f) => (
                  <li key={f.path} className="text-mono">
                    {f.path} <span className="text-sec">({fmtBytes(f.size)})</span>
                  </li>
                ))}
              </ul>
            </>
            ...
            <>
              ...
              <div className="doc-result-preview__content">
                <Markdown remarkPlugins={[remarkGfm]}>{status.content || "_(empty)_"}</Markdown>
              </div>
            </>
          )}
        </div>
