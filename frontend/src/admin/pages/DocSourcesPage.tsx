...
function ValidateButton({ id }: { id: string }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: setResult,
    onError: () => setResult({ ok: false, level: "error", message: "Request failed" }),
  });
  return (
    <div className="db-test">
      ...
      >
        {mutation.isPending ? <i className="fa-solid fa-spinner fa-spin" /> : "Validate"}
      </button>
      {result && (
...
export function DocSourcesPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin", "docsources"] }),
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Documentation</h1>
          <p>Folders under documentations/ exposed to the agent and the docs viewer</p>
        </div>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <p className="text-sec">Register a folder or generate documentation with AI.</p>
                ...
                {sources.map((src) => (
                  <tr key={src.id}>
                    ...
                    <td className="text-mono text-sm">
                      {(src.file_patterns && src.file_patterns.length
                        ? src.file_patterns
                        : ["*.md"]
                      ).join(", ")}
                    </td>
