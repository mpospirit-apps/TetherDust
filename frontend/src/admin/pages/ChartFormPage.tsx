...
export function ChartFormPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!code) {
        el.innerHTML = '<div class="chart-preview-empty">Write d3 code to render the chart.</div>';
        return;
      }
      const err = runChartCode(el, code, previewData.data, buildTheme());
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "charts", dashboardId] });
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>Write a read-only SQL query and the d3 code that renders its result</p>
        </div>
      ...
      <form id="chart-form" onSubmit={onSubmit}>
        <div className="chart-edit-grid">
          <div className="card">
            ...
            <FormField label="SQL query" help="Read-only SELECT that produces the chart data.">
              <textarea
                className="form-control"
                rows={8}
                style={MONO}
            ...
            <div className="form-actions">
              <button type="submit" className="btn btn-primary" disabled={save.isPending}>
                {save.isPending ? "Saving…" : isEdit ? "Save Changes" : "Create Chart"}
              </button>
              <Link to={`/admin/dashboards/${dashboardId}`} className="btn btn-secondary">
                Cancel
              </Link>
            </div>
          </div>
          ...
          <div className="chart-edit-sidebar">
            <div className="chart-preview-card">
              ...
              <div className="chart-preview-card__body">
                ...
                />
                {previewError && (
                  <div className="chart-preview-error is-visible">{previewError}</div>
                )}
              </div>
            </div>
...
function ChartAiPanel({ chartId, onDone }: { chartId: string; onDone: () => void }) {
  const { messages, streaming, statusText, connected, send } = useChartEditSocket({
    chartId,
    onDone,
  });
  ...
  return (
    <div className="chart-aichat-card">
      <div className="chart-aichat-card__header">
        ...
        <span style={{ color: connected ? "var(--c-lime)" : "var(--text-muted)" }}>
        ...
        {messages.length === 0 ? (
          <div className="chart-aichat-empty">
            <div className="chart-aichat-empty__intro">
              Ask the agent to tweak the query or the d3 code — e.g. “make it a horizontal bar
              chart” or “group by month”.
            </div>
          </div>
        ) : (
          messages.map((m, i) => (
            <div key={i} className={`chart-aichat-msg chart-aichat-msg--${m.role}`}>
      ...
      <form className="chart-aichat-form" onSubmit={onSubmit}>
        ...
        />
        <button type="submit" className="btn btn-primary btn-sm" disabled={streaming || !connected}>
          Send
        </button>
      </form>
    </div>
