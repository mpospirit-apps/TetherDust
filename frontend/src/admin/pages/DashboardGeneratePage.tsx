...
import { AgentSelect, SourceSelect, type SourceSelection } from "./DocGenShared";
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>The agent explores your data and builds a dashboard of D3 charts</p>
        </div>
      ...
      {logId ? (
        <div className="card">
          {!s ? (
            ...
            <div className="doc-loading-state">
              ...
              <p>Building dashboard… {s.charts_created ? `${s.charts_created} charts so far` : ""}</p>
              {s.agent_output && (
                <p className="doc-loading-elapsed">{s.agent_output.slice(0, 200)}</p>
              )}
            </div>
          ) : s.status === "failed" ? (
            <div className="flash flash-error">Generation failed: {s.error || "unknown error"}</div>
          ) : (
            <div>
              ...
              <div className="form-actions" style={{ marginTop: "var(--md)" }}>
                {s.dashboard_id && (
                  <Link to={`/dashboards/${s.dashboard_id}`} className="btn btn-primary">
                    View dashboard
                  </Link>
                )}
        ...
        <form onSubmit={onSubmit} className="card" style={{ maxWidth: 760 }}>
          ...
          <div className="doc-req-row" style={{ gridTemplateColumns: "2fr 1fr" }}>
