...
import { listDocGenLogs, type DocGenLog } from "../../api/docs";
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id}>
                    ...
                    <td>
                      <span className="type-badge">{log.is_library ? "library" : log.doc_type}</span>
                    </td>
                    <td>
                      <span className={statusBadge(log.status)}>{log.status.toUpperCase()}</span>
                      {log.status === "failed" && log.error_message && (
                        <div className="text-sm text-sec truncate">{log.error_message}</div>
                      )}
                    </td>
