...
function TestButton({ id }: { id: string }) {
  ...
  return (
    <div className="db-test">
      ...
      >
        {mutation.isPending ? <i className="fa-solid fa-spinner fa-spin" /> : "Test"}
      </button>
      {result && (
        <span className={result.ok ? "badge badge-success" : "badge badge-error"}>
...
export function DatabasesPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin", "databases"] }),
  });
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <p className="text-sec">Add a database connection to get started.</p>
              ...
              <tbody>
                {connections.map((conn) => (
                  <tr key={conn.id}>
                    ...
                    <td>
                      <div className="flex-gap">
                        <Link to={`/admin/databases/${conn.id}`} className="btn btn-ghost btn-sm">
                          Edit
                        </Link>
                        <button
                          type="button"
