...
export function MCPServersPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin", "mcp-servers"] }),
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>MCP Servers</h1>
          <p>Register custom MCP servers (remote HTTP or local subprocess) for the agent.</p>
        </div>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <p className="text-sec">Register a custom MCP server to extend the agent's tools.</p>
                ...
                {servers.map((s) => (
                  <tr key={s.id}>
                    <td>
                      ...
                      {s.description && <div className="text-sec text-sm">{s.description}</div>}
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="flex-gap">
                        <Link to={`/admin/mcp-servers/${s.id}`} className="btn btn-ghost btn-sm">
                          View
                        </Link>
                        {!s.is_builtin && (
                          <>
