...
export function AgentFormPage() {
  ...
  const meta = useQuery({ queryKey: ["admin", "agent-types"], queryFn: getAgentTypes });
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      if (isClaudeCode && form.oauth_token) payload.oauth_token = form.oauth_token;
      ...
      <div>
        ...
        {meta.isLoading ? (
          ...
          (m?.categories ?? []).map((cat) => (
            <div className="card" key={cat.title} style={{ marginBottom: "var(--md)" }}>
  ...
  const typeLabel = isEdit
    ...
    : (m?.categories.flatMap((c) => c.types).find((t) => t.value === type)?.label ?? type);
    ...
    <div>
      ...
      {isCodexAuth && (
        <div className="card" style={{ marginBottom: "var(--md)" }}>
          ...
          <p className="text-sec" style={{ margin: "0 0 var(--md)", lineHeight: 1.6 }}>
            TetherDust authenticates with a ChatGPT subscription (Plus/Pro/Enterprise). Sign in once
            below — the credential is stored encrypted and refreshed automatically.
          </p>
          {isEdit && id ? (
            <CodexDeviceLogin agentId={id} authInfo={existing.data?.auth_info ?? null} />
          ) : (
            <p className="text-sec" style={{ margin: 0, lineHeight: 1.6 }}>
              Save this agent first, then return to the edit screen and use{" "}
              <strong>Sign in to ChatGPT</strong> to complete device-code login from the browser.
            </p>
          )}
        </div>
      )}
      ...
      <form onSubmit={onSubmit}>
        <div className="card">
          ...
          {!isDirect && (
            <FormField label="Service URL" help="Override the agent gateway URL. Blank = default.">
              <input
                className="form-control"
          ...
          {isApiKey && (
            <FormField label="API Key" help={hasKey ? "Leave blank to keep existing." : "Required."}>
              <input
                className="form-control"
                type="password"
          ...
          {isClaudeCode && (
            <FormField
              label="OAuth Token"
              help={hasToken ? "Leave blank to keep existing." : "From `claude setup-token`."}
            >
              <input
                className="form-control"
                type="password"
                autoComplete="new-password"
                placeholder={hasToken ? "••••••••  (leave blank to keep)" : "sk-ant-oat…"}
...
function CodexDeviceLogin({
  ...
  const [prompt, setPrompt] = useState<{ url: string; code: string } | null>(null);
    ...
    <div>
      {authInfo && (
        ...
        >
          ...
          <div style={{ fontSize: "var(--text-sm)", lineHeight: 1.5 }}>
            <div>
              Signed in{authInfo.email && <> as <strong>{authInfo.email}</strong></>}
              {authInfo.plan && (
                <span
                  style={{
      ...
      {authInfo && (
        <div className="helptext" style={{ marginTop: "var(--xs)" }}>
          Sign in again to replace the stored credential (e.g. after expiry or to switch accounts).
        </div>
      )}

      <div style={{ marginTop: "var(--sm)", fontSize: "var(--text-sm)", lineHeight: 1.6 }}>
