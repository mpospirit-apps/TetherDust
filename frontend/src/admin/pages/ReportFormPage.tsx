...
const STEPS: WizardStepDef[] = [
	{
		...
		description: "Name the report and set whether it's active.",
	},
	{
		key: "configuration",
		label: "Configuration",
...
export function ReportFormPage() {
	...
	useEffect(() => {
		...
		setForm({
			...
			schedule_interval_minutes:
				r.schedule_interval_minutes != null
					? String(r.schedule_interval_minutes)
					: "",
			schedule_time: r.schedule_time ? r.schedule_time.slice(0, 5) : "",
			schedule_day_of_week:
				r.schedule_day_of_week != null ? String(r.schedule_day_of_week) : "",
			schedule_day_of_month:
				r.schedule_day_of_month != null ? String(r.schedule_day_of_month) : "",
	...
	function buildPayload(): ReportDefinitionInput {
		return {
			...
			schedule_day_of_week:
				form.schedule_type === "weekly"
					? toNumberOrNull(form.schedule_day_of_week)
					: null,
			schedule_day_of_month:
				form.schedule_type === "monthly"
					? toNumberOrNull(form.schedule_day_of_month)
					: null,
