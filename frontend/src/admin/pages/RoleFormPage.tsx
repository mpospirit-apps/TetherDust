...
export function RoleFormPage() {
  ...
  const grants = useQuery({ queryKey: ["admin", "role-grants"], queryFn: getRoleGrants });
      ...
      <form onSubmit={onSubmit}>
        <div className="form-split">
          <div className="card">
            ...
            <FormField label="Max Row Limit" help="Max rows per query for this role.">
              <input
                className="form-control"
                type="number"
                value={form.max_row_limit ?? ""}
                onChange={(e) =>
                  set("max_row_limit", e.target.value === "" ? null : Number(e.target.value))
                }
              />
            </FormField>
            <FormCheckbox label="Can chat" checked={form.can_chat} onChange={(v) => set("can_chat", v)} />
            <FormCheckbox
