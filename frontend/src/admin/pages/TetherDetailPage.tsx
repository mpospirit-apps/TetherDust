...
export function TetherDetailPage() {
  ...
  const regenerate = useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "tether-status", tetherId] });
      void queryClient.invalidateQueries({ queryKey: ["admin", "tether-versions", tetherId] });
    },
    ...
    <div>
      <div className="page-header">
        ...
        <div className="flex-gap">
          ...
          >
            <i className="fa-solid fa-wand-magic-sparkles" /> {running ? "Generating…" : "Regenerate"}
          </button>
        </div>
      </div>
                  ...
                  <tr key={v.id}>
                    ...
                    <td>
                      ...
                      {v.status === "failed" && v.error_message && (
                        <div className="text-sm text-sec truncate" title={v.error_message}>
                          {v.error_message}
                        </div>
                      )}
                    </td>
                    <td className="text-sm">{new Date(v.started_at).toLocaleString()}</td>
                    <td>{v.execution_time_ms != null ? `${v.execution_time_ms}ms` : "—"}</td>
                    <td className="text-sm text-sec">{v.triggered_by ?? "—"}</td>
                    <td>
                      {v.is_current && (
                        <Link to={`/tethers/${t.id}`} className="btn btn-ghost btn-sm">
                          View
                        </Link>
                      )}
                    </td>
