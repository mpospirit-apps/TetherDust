...
export function CodebasesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    refetchInterval: (query) =>
      (query.state.data?.results ?? []).some((c) => c.sync_status === "syncing") ? 3000 : false,
  });
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Codebases</h1>
          <p>Source repositories the agent reads on demand (GitHub, no clone)</p>
        </div>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <h3>No Codebases</h3>
            <p className="text-sec">Add a GitHub repository for the agent to browse.</p>
                ...
                {codebases.map((c) => (
                  <tr key={c.id}>
                    <td>
                      ...
                      {c.description && (
                        <div className="text-sm text-sec truncate">{c.description}</div>
                      )}
                    </td>
                    <td className="text-sm">{c.repo_url}</td>
                    <td className="text-sm">{c.branch || c.default_branch || "default"}</td>
                    <td>
                    ...
                    <td>
                      <div className="flex-gap">
                        ...
                        <Link to={`/admin/codebases/${c.id}`} className="btn btn-ghost btn-sm">
                          Edit
                        </Link>
                        <button
                          type="button"
