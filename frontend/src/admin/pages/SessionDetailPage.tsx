		...
		<div style={rowStyle}>
			...
			<div style={{ flex: 1, minWidth: 0 }}>
				...
				{msg.sources_used.length > 0 && (
					<div
						className="flex-gap"
						style={{ flexWrap: "wrap", marginTop: "var(--xs)" }}
					>
						{msg.sources_used.map((s) => (
							<span key={s.uri ?? s.name} className="badge badge-muted">
								@{s.name ?? s.uri}
							</span>
						))}
					</div>
				...
				{msg.tools_used.length > 0 && (
					...
					>
						{msg.tools_used.map((t) => (
							<span key={t} className="badge badge-muted text-mono">
								{t}
							</span>
						))}
					</div>
			...
			<div className="page-header">
				<div>
					...
					<p className="text-mono">
						{data ? `${data.id} — ${data.user ?? "—"}` : "Transcript"}
					</p>
				</div>
