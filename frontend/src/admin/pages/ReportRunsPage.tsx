              ...
              <tbody>
                {executions.map((ex) => (
                  <tr key={ex.id}>
                    ...
                    <td className="text-sm">{new Date(ex.started_at).toLocaleString()}</td>
                    <td>{ex.execution_time_ms != null ? `${ex.execution_time_ms}ms` : "—"}</td>
                    <td>{ex.row_count ?? "—"}</td>
                    <td className="text-sm text-sec">{ex.triggered_by ?? "Scheduled"}</td>
                    <td>
                      <Link to={`/admin/report-runs/${ex.id}`} className="btn btn-ghost btn-sm">
                        View
                      </Link>
                    </td>
                  </tr>
