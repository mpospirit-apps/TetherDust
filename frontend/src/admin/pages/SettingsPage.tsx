...
function GeneralSettingsCard() {
	...
	useEffect(() => {
		...
		setForm({
			...
			docgen_timeout:
				data.docgen_timeout == null ? "" : String(data.docgen_timeout),
			doclibgen_timeout:
				data.doclibgen_timeout == null ? "" : String(data.doclibgen_timeout),
			chartgen_timeout:
				data.chartgen_timeout == null ? "" : String(data.chartgen_timeout),
			max_row_limit:
				data.max_row_limit == null ? "" : String(data.max_row_limit),
			hot_reload_interval:
				data.hot_reload_interval == null
					? ""
					: String(data.hot_reload_interval),
		});
	}, [data]);
...
function SmtpSettingsCard() {
	...
	useEffect(() => {
		...
		setForm({
			...
			email_max_rows:
				data.email_max_rows == null ? "" : String(data.email_max_rows),
		});
	...
	return (
		<form className="card" onSubmit={onSubmit}>
			...
			<div className="form-grid">
				...
				<FormField
					label="Password"
					help={hasPassword ? "Leave blank to keep existing." : undefined}
				>
					<input
						className="form-control"
						type="password"
			...
			>
				...
				{testMsg && (
					<span
						className={testMsg.ok ? "badge badge-success" : "badge badge-error"}
					>
						{testMsg.text}
					</span>
				)}
