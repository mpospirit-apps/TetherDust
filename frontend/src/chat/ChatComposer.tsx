...
export function ChatComposer({
	...
	const activeItems: (DocResource | ChatPrompt)[] =
		mode === "doc" ? docItems : promptItems;
	...
	function addResource(res: DocResource) {
		setChips((cs) =>
			cs.some((c) => c.kind === "doc" && c.res.uri === res.uri)
				? cs
				: [...cs, { kind: "doc", res }],
		);
	}
							...
							docItems.map((res, i) => (
								...
								>
									...
									<span className="source-desc">
										...
										{(() => {
											let acc = "";
											return res.path.split("/").map((seg, idx) => {
												acc = acc ? `${acc}/${seg}` : seg;
												return (
													<Fragment key={acc}>
														{idx > 0 && <Arrow />}
