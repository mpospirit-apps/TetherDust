...
// Stable per-message id used as the React list key. Append-only chat means a
// fresh id at creation time is a stable identity for the message's lifetime.
...
export function useChatSocket({
	...
	useEffect(() => {
		// This effect's connection generation. A later connKey (new chat / switched
		// session) supersedes it; guard async socket callbacks so a torn-down
		// connection can't write state over the live one.
		const generation = connKey;
		const isStale = () => connKeyRef.current !== generation;
		...
		ws.onmessage = (event) => {
			...
			switch (data.type) {
				...
				case "stream_chunk":
					...
					setMessages((ms) =>
						setLastAssistant(ms, { content: streamRef.current }),
					);
					break;
				case "stream_end":
					setStreaming(false);
					...
					setMessages((ms) =>
						setLastAssistant(
							ms,
							{
								content: data.content || streamRef.current,
								tools: data.tools,
								pending: false,
							},
							true,
	...
	const send = useCallback(
		(
			...
			const promptsInfo = prompts.map((p) => ({
				name: p.name,
