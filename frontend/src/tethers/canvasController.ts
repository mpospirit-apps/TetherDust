...
export function createTetherCanvas(root: HTMLElement, graph: TetherGraph): () => void {
  ...
  const cardsLayer = root.querySelector<HTMLElement>(".tether-cards-layer");
  if (!svg || !cardsLayer) return () => ac.abort();
  ...
  const searchInput = root.querySelector<HTMLInputElement>(".tether-search--popup");
  const filterButtons = root.querySelectorAll<HTMLElement>(".tether-filters [data-rel]");
  const fitBtn = root.querySelector<HTMLElement>(".tether-fitbtn");
  const spreadInput = root.querySelector<HTMLInputElement>("#tether-spread-range");
  ...
  function prepareData(g: TetherGraph): void {
    state.allNodes = g.nodes.map((n) => ({ ...n, x: 0, y: 0, _w: MIN_CARD_W, _h: 0 }) as RuntimeNode);
    state.allEdges = g.edges.map((e, i) => ({ ...e, id: `e${i}` }) as RuntimeEdge);
  ...
  function buildCardEl(c: RuntimeNode): HTMLElement {
    ...
    if (c.kind === "db-table" && c.schema) header.appendChild(chip(c.schema, "tcard__chip"));
    if (c.kind === "code-file" && c.language) header.appendChild(chip(c.language, "tcard__chip"));
    ...
    for (const row of c._visibleRows!) ul.appendChild(buildRowEl(c, row));
  ...
  function positionCards(): void {
    for (const c of state.containers) {
      const el = cardsLayer!.querySelector<HTMLElement>(`[data-node-id="${cssEscape(c.id)}"]`);
      if (!el) continue;
  ...
  function initSimulation(): void {
    ...
    state.simulation = d3
      ...
      .force(
        "xLane",
        d3
          .forceX<RuntimeNode>((d) => (d._side === "code" ? state.CODE_LANE_X : state.DB_LANE_X))
          .strength(0.1)
      )
      .force("y", d3.forceY<RuntimeNode>((state.H + 52) / 2).strength(0.02))
      .force("collide", d3.forceCollide<RuntimeNode>((d) => d._radius || 100).strength(0.9).iterations(2))
      .alpha(1)
      .alphaDecay(0.04);
  ...
  function filterEdges(): Set<string> {
    ...
    for (const e of state.allEdges) if (state.activeRels.has(e.relationship)) ids.add(e.id);
    return ids;
  }

  function isDimmed(e: RuntimeEdge): boolean {
    if (!state.activeRels.has(e.relationship)) return true;
    if (state.focusIds && !(state.focusIds.has(e.source_id) || state.focusIds.has(e.target_id)))
      return true;
    if (state.searchTerm) {
  ...
  function applyCardDimming(): void {
    for (const c of state.containers) {
      ...
      if (state.searchTerm) {
        const hay = (c.label + " " + (c._kids || []).map((k) => k.label).join(" ")).toLowerCase();
    ...
    state.tipTimer = window.setTimeout(() => {
      ...
      if ("relationship" in target && target.relationship) {
        ...
        if (target.description)
          ...
          fragments.push(`<span class="tether-tooltip__desc">${target.data_type}</span>`);
        else if (target.signature)
          fragments.push(`<span class="tether-tooltip__desc">${escapeHtml(target.signature)}</span>`);
      }
  ...
  function refreshHighlights(): void {
    ...
    cardsLayer!.querySelectorAll(".trow").forEach((el) => el.classList.remove("is-selected"));
    edgesG.querySelectorAll(".tether-edge").forEach((el) => el.classList.remove("is-selected"));
    if (!state.selected) return;
    if (state.selected.kind === "node") {
      const n = state.selected.ref;
      if (n.kind === "code-file" || n.kind === "db-table") {
        const el = cardsLayer!.querySelector(`.tcard[data-node-id="${cssEscape(n.id)}"]`);
        if (el) el.classList.add("is-selected");
      } else {
        const el = cardsLayer!.querySelector(`.trow[data-row-id="${cssEscape(n.id)}"]`);
        if (el) el.classList.add("is-selected");
      }
    } else {
  ...
  function panelHeader(eyebrow: string, title: string, accent?: string): HTMLElement {
  ...
  function setupInfoPanel(g: TetherGraph): void {
    ...
    if (g.codebase_summary) infoPanel.appendChild(buildInfoSection("Code", g.codebase_summary));
    if (g.database_summary) infoPanel.appendChild(buildInfoSection("Database", g.database_summary));
  }
  ...
  function centerOnFocused(): void {
    ...
    const focused = state.containers.filter(
      (c) => state.focusIds!.has(c.id) || (c._kids || []).some((k) => state.focusIds!.has(k.id))
    );
  ...
  function bindToolbar(): void {
    ...
    const searchToggle = root.querySelector<HTMLElement>("#tether-search-toggle");
    const filterToggle = root.querySelector<HTMLElement>("#tether-filter-toggle");
    ...
    const spreadToggle = root.querySelector<HTMLElement>("#tether-spread-toggle");
    ...
    if (spreadInput) {
      ...
      spreadInput.addEventListener("input", () => {
        ...
        const charge = state.simulation?.force("charge") as
    ...
    if (gapInput) {
      ...
      gapInput.addEventListener("input", () => {
        ...
        const xForce = state.simulation?.force("xLane") as d3.ForceX<RuntimeNode> | undefined;
        if (xForce) {
