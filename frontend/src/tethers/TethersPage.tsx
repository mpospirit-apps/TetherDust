...
function TetherViewer({ id }: { id: string }) {
  const detail = useQuery({ queryKey: ["tether", id], queryFn: () => getTether(id) });
...
export function TethersPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["tethers"], queryFn: getTethers });
          ...
          {isLoading ? (
            ...
            tethers.map((t) => (
              <NavLink
                key={t.id}
                to={`/tethers/${t.id}`}
                className={({ isActive }) => (isActive ? "docs-file-btn active" : "docs-file-btn")}
              >
      ...
      <div className="docs-content-area">
        {id ? (
          ...
          <div className="docs-empty-state">
            ...
            <p>Select a tether from the sidebar to explore its code ↔ database graph.</p>
          </div>
        )}
      </div>
    </div>
