...
export interface ReportDefinition {
  ...
  latest_run: { id: string; status: string; started_at: string; row_count: number | null } | null;
  created_at: string;
  updated_at: string;
}
...
export function createReport(data: ReportDefinitionInput): Promise<ReportDefinition> {
  return apiFetch(RPT_BASE, { method: "POST", body: JSON.stringify(data) });
}
export function updateReport(id: string, data: ReportDefinitionInput): Promise<ReportDefinition> {
  return apiFetch(`${RPT_BASE}${id}/`, { method: "PATCH", body: JSON.stringify(data) });
}
...
export function listExecutions(definitionId?: string): Promise<Paginated<ReportExecutionRow>> {
  const qs = definitionId ? `?definition=${encodeURIComponent(definitionId)}` : "";
