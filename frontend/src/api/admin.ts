...
export function createDatabase(data: DatabaseInput): Promise<DatabaseConnection> {
  return apiFetch(DB_BASE, { method: "POST", body: JSON.stringify(data) });
}

export function updateDatabase(id: string, data: DatabaseInput): Promise<DatabaseConnection> {
  return apiFetch(`${DB_BASE}${id}/`, { method: "PATCH", body: JSON.stringify(data) });
}
...
export type SmtpSettingsInput = Omit<SmtpSettings, "has_password"> & { smtp_password?: string };

export function getGeneralSettings(): Promise<GeneralSettings> {
  return apiFetch("/api/v1/admin/settings/general/");
}

export function updateGeneralSettings(data: GeneralSettings): Promise<GeneralSettings> {
  return apiFetch("/api/v1/admin/settings/general/", { method: "PUT", body: JSON.stringify(data) });
}
...
export function updateSmtpSettings(data: SmtpSettingsInput): Promise<SmtpSettings> {
  return apiFetch("/api/v1/admin/settings/smtp/", { method: "PUT", body: JSON.stringify(data) });
}
...
export function updateRole(id: string, data: RoleInput): Promise<Role> {
  return apiFetch(`${ROLE_BASE}${id}/`, { method: "PUT", body: JSON.stringify(data) });
}
...
export function updateUser(id: number, data: UserInput): Promise<AdminUser> {
  return apiFetch(`${USER_BASE}${id}/`, { method: "PATCH", body: JSON.stringify(data) });
}
...
export function getDefaultPrompt(agentType: string): Promise<{ system_prompt: string }> {
  return apiFetch(`${AGENT_BASE}default-prompt/?agent_type=${encodeURIComponent(agentType)}`);
}
...
export function updateAgent(id: string, data: AgentInput): Promise<Agent> {
  return apiFetch(`${AGENT_BASE}${id}/`, { method: "PATCH", body: JSON.stringify(data) });
}
...
export function getDeviceLoginStatus(id: string, loginId: string): Promise<DeviceLoginStatus> {
