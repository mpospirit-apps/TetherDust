...
import { getChartData, type ChartData, type ChartView } from "../api/dashboards";
...
export function ChartCard({
  ...
  useEffect(() => {
    ...
    renderChart(el, chart.custom_d3_code, data.data, buildTheme());
    ...
    <div className="chart-card" style={{ gridColumn: `span ${chart.width}` }}>
      <div className="chart-card__header">
        ...
        <div className="chart-card__meta">
          ...
          >
            <i className={`fa-solid fa-rotate${refresh.isPending ? " spinning" : ""}`} />
          </button>
        </div>
      </div>
      <div className="chart-card__body">
        <div className="chart-container" style={{ height: `${chart.height}px` }} ref={ref} />
      </div>
    </div>
  );
}
