...
// Stable per-message id used as the React list key (append-only chat log).
...
export function useChartEditSocket({
	...
	useEffect(() => {
		...
		const ws = new WebSocket(
			`${proto}//${window.location.host}/ws/chart-edit/${chartId}/`,
		);
		wsRef.current = ws;
			...
			if (streamingRef.current) {
				...
				setMessages((ms) => [
					...ms.filter((m) => !m.pending),
					{
						id: newMessageId(),
						role: "error",
						content: "Connection lost during agent response.",
					},
				]);
			}
		};
		ws.onmessage = (event) => {
			...
			switch (data.type) {
				...
				case "stream_chunk":
					...
					setMessages((ms) =>
						patchLastAssistant(ms, { content: streamRef.current }),
					);
					break;
				case "stream_end":
					streamingRef.current = false;
