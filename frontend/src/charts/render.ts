...
// `mode` is the active light/dark theme. Callers pass it explicitly (from React
// state) so a theme toggle re-runs their render effect; the CSS-var reads below
// resolve against the already-applied `data-theme` for that mode.
export function buildTheme(mode: string): ChartTheme {
	const palette = [
		readCssVar("--c-red"),
		readCssVar("--c-cyan"),
		readCssVar("--c-lime"),
...
export function runChartCode(
	...
	container.innerHTML = "";
	try {
		new Function("data", "container", "d3", "theme", d3Code)(
			data,
			container,
			window.d3,
			theme,
...
export function timesince(iso: string | null | undefined): string {
	...
	if (Number.isNaN(then)) return "Not cached";
