...
function groupSortKey(label: string): [number, number] {
  ...
  if (!Number.isNaN(parsed.getTime())) return [3, -(parsed.getFullYear() * 12 + parsed.getMonth())];
  return [4, 0];
}
...
type Content = { kind: "latest" } | { kind: "history" } | { kind: "execution"; execId: string };
...
function LatestView({
  ...
  if (isError || !data) return <p className="text-sec">Failed to load report.</p>;
  if (!data.execution) {
    return (
...
function HistoryView({
  ...
  if (isLoading) return <Loading />;
  if (isError || !data) return <p className="text-sec">Failed to load history.</p>;
  return (
    <div>
            ...
            <tbody>
              {data.executions.map((ex) => (
                <tr key={ex.id}>
                  ...
                  <td>{ex.execution_time_ms != null ? `${ex.execution_time_ms}ms` : "—"}</td>
                  <td>{ex.row_count ?? "—"}</td>
                  <td>
                    <button
...
function ExecutionView({ execId }: { execId: string }) {
  ...
  if (isLoading) return <Loading />;
  if (isError || !data) return <p className="text-sec">Failed to load execution.</p>;
  return (
    <ReportResultTable
      report={data.report}
      execution={data.execution}
      ...
      <div className="docs-content-area">
        <div className="docs-content">
          {!selectedId ? (
            ...
            <HistoryView
              reportId={selectedId}
              onOpenExecution={(execId) => setContent({ kind: "execution", execId })}
            />
