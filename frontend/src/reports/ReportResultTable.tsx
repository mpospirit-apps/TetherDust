...
function EmailButton({ executionId }: { executionId: string }) {
  ...
  return (
    <button className="btn btn-ghost btn-sm" onClick={() => void send()} disabled={state === "sending"}>
      {state === "sending" ? (
...
export function ReportResultTable({
  ...
  const hasData = execution.column_names.length > 0 && execution.rows.length > 0;
      ...
      <div className="report-meta">
        ...
        <div className="report-info">
          ...
          {execution.row_count != null && (
            <span className="badge badge-muted">{execution.row_count} rows</span>
          )}
          {execution.execution_time_ms != null && (
            <span className="badge badge-muted">{execution.execution_time_ms}ms</span>
          )}
          ...
          {onShowHistory && !isPreview && (
            <button className="btn btn-ghost btn-sm" onClick={onShowHistory}>
        ...
        <>
          {truncated && (
            <div className="report-preview-notice">
              ...
              Showing first {execution.rows.length} of {total} rows. Download the full report using
              the CSV or Excel buttons above.
            </div>
          )}
