...
class TetherForm(_BaseForm):
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        ...
        self.fields["code_source"].choices = choices
    ...
    def clean_code_source(self) -> tuple[str, Any]:
        ...
        if kind == _CODEBASE_PREFIX:
            obj = Codebase.objects.filter(pk=pk, is_active=True).first()
            if obj is None:
                raise forms.ValidationError("Selected codebase is no longer available.")
            return (_CODEBASE_PREFIX, obj)
        if kind == _CODEBASE_DOC_PREFIX:
            obj = DocumentationSource.objects.filter(
            ...
            return (_CODEBASE_DOC_PREFIX, obj)
