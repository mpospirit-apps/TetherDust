...
source is. Staff register a connection in the console; from then on the agent can
...
The console writes connection config to the app database; the built-in MCP
...
`core/models/connections.py` → `DatabaseConnection`.
...
3. **Test** (`console/views/database.py` → `database_test_view`) runs a
...
2. **App DB over SQLAlchemy** — when running standalone with `ADMIN_DATABASE_URL`
   set, it queries the `core_databaseconnection` table and Fernet-decrypts
   passwords. This is how the `mcp` container reads console-managed connections
