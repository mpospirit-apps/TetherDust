...
| `can_manage_users` | `False` | Allows a non-superuser **staff** member to create and edit users in the console. This flag does not make a user staff and does not grant console access by itself. |
| `is_active` | `True` | Marks the role active/inactive in the console. This flag is not currently a request-time deny-all gate by itself; remove permissions or reassign users to revoke access. |
| `is_admin_role` | `False` | Console-managed admin flag. Saving roles or users through the console syncs non-superuser users with this role to `User.is_staff=True`, grants console access, and bypasses role allow-lists. See [Admin access paths](#admin-access-paths). |
...
`core/models/auth.py` → `UserProfile`.
...
- `None` — unrestricted; all objects of that type are accessible. Staff users return `None`, and users assigned an admin role through the console become staff.
...
(`core/consumers/permissions.py`) and also registered as a per-request token
filter at the MCP server (`core/agents/mcp_filter.py`).
...
assigned role. A staff user also has access to the console (`/console/`).

Staff status is set by assigning an admin role through the TetherDust console,
...
`is_admin_role` is the console-level shortcut for staff/admin access. When a role
...
the console, assigning an admin role applies the same staff sync.

Because console access is staff-gated, users assigned an admin role through the
supported console flows can access `/console/` and receive unrestricted portal
and agent access. There is no supported portal-only admin role in the current UI.

If `is_admin_role` is changed outside the console forms without syncing
...
| `is_admin_role=True`, `can_manage_users=False` | Yes. The console syncs assigned non-superusers to staff. | No, unless the user is a superuser. | Bypassed. |
...
| **Console** | `is_staff = True`. Assigning an admin role through the console sets this flag for non-superusers. |
...
Both are managed from the console.
...
- Checking **Admin role** hides the allow-list controls because the console syncs assigned non-superusers to staff, making those allow-lists irrelevant for them.
- Checking **Can manage users** without **Admin role** is useful only for users who are already staff by some other path; it does not grant console access.
...
**1. The WebSocket consumer** (`core/consumers/permissions.py`) — reads the
user's `UserProfile` before the agent starts and builds the allow-lists that are
passed to the agent.

**2. The MCP server token filter** (`core/agents/mcp_filter.py` +
