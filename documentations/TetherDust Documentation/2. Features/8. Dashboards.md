...
10. [Managing dashboards in the console](#managing-dashboards-in-the-console)
...
flowchart TD
    subgraph console["Console (staff)"]
        Gen["console/views/dashboard.py\ngenerate (background thread)"]
...
Three models in `core/models/dashboards.py`.
...
From `console/views/dashboard.py`:
...
directly to the `core_dashboard` / `core_chart` tables — see
...
Charts don't have to be AI-generated. The console chart form
(`console/views/dashboard.py`) lets staff write the title, SQL, database, layout,
...
(console `chart_data_view`, portal `chart_data_api_view`) follow the same logic:
...
before caching. The portal endpoint additionally checks that the requesting
...
The portal viewer (`portal/views/dashboards.py`, login-required) is at
`/dashboards/`:
...
role from the dashboard's console form (**Allowed roles**). Staff bypass role
filtering; all authoring actions are `@staff_member_required`.

---

## Managing dashboards in the console

Staff manage dashboards at **Console → Dashboards** (`console/views/dashboard.py`):
