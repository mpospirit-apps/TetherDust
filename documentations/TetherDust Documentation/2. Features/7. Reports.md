...
portal. No AI agent is involved; reports are plain SQL executed directly.
...
10. [Managing reports in the console](#managing-reports-in-the-console)
...
Two models in `core/models/reports.py`.
...
Two Celery tasks drive scheduled runs (`core/tasks.py`):
...
- **`execute_report_task`** — executes a single report (soft limit 300s, hard
  360s). Manual runs from the console call the same task.
...
(`core/engines/result_storage.py`). Each execution gets a directory under
...
`validate_sql()` (`core/engines/report_engine.py`):
...
The same validator backs the console's `ChartForm` and the chart preview, so the
...
The portal viewer (`portal/views/reports.py`, login-required) is at `/reports/`:
...
Email delivery uses `core/engines/email_service.py` over SMTP. Two paths reach
it:
...
Grant a report to a role from the report's console form (**Allowed roles**).
Staff bypass role filtering everywhere. All authoring actions are
`@staff_member_required`.

---

## Managing reports in the console

Staff manage reports at **Console → Reports** (`console/views/report.py`):
