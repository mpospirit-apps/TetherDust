...
7. [Managing custom servers in the console](#managing-custom-servers-in-the-console)
...
`MCPServerConfiguration` (`core/models/connections.py`) backs both kinds, plus
the built-in mirror.
...
1. **On WebSocket connect**, the chat consumer resolves the user's allowed
   ...
   (`core/consumers/permissions.py`). For each:
...
2. **On each turn**, the consumer passes the list as `custom_mcp_servers=` to
   `agent.chat()` (`portal/consumers/chat.py`).
3. **The agent** forwards it to the gateway in the `/chat` payload
   (`core/agents/codex.py`), which renders MCP server config the CLI understands
...
this list (`core/models/auth.py`). Tool-level access within a server still
applies on top — a granted server's individual tools are gated by the role's
`allowed_tools` and the tool's `is_enabled` flag.

---

## Managing custom servers in the console

Staff manage servers at **Console → MCP Servers** (`console/views/mcp_server.py`):
...
| **Local server config** (command, args, env, active toggle) | Live — the console pings the local-mcp `/reload` on save; the proxy re-syncs subprocesses. |
