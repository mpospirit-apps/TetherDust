...
1. **The running service** — the `mcp` container. It is a long-running Python process (`python -m mcp_server.server`) that actually executes the tools. The agent connects to it over HTTP at the URL given by `MCP_BASE_URL` (default `http://mcp:8001`), and this is the *only* place tool calls are executed and enforced.
...
The built-in server registers fifteen tools (`tetherdust/mcp_server/tools/__init__.py`, `register_tools()`):
...
Two models in `tetherdust/web/core/models/connections.py` back the mirror.
...
Because the built-in rows are reference data, they are seeded idempotently rather than hand-created. The canonical definition lives in `tetherdust/web/core/builtin_mcp.py` (`BUILTIN_SERVER_NAME`, `BUILTIN_TOOLS`, and `ensure_builtin_mcp()`), and it is wired to Django's `post_migrate` signal in `core/apps.py`:
