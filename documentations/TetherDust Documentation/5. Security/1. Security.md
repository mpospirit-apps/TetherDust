...
`engine/models/_encryption.py` provides two helpers:
...
mechanism (`engine/agents/mcp_filter.py`):
...
2. Django hands the agent a tokenized MCP URL: `http://tdmcp:8001/mcp/<token>`.
...
- The Django admin (`/admin/`) is available to superusers for emergency access
  but is not used for routine administration (use the management instead).
...
- [ ] All database connections have **Read-only** checked in the management.
