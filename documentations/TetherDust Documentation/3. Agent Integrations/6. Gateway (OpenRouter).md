...
flowchart TD
    ...
    subgraph web["Django + Channels · web:8000"]
        ...
        Agent["OpenRouterAgent ⊂ OpenAICompatibleAgent<br/>engine/agents/direct_api.py<br/>in-process tool loop — identical to Direct API"]
    end
...
Registered under the `openrouter` agent type in `engine/agents/__init__.py` and
...
management: **Agents → Add Agent → "OpenRouter (Gateway)"**, then:
