...
**`system_prompt` column on `AgentConfiguration`** (`core/models/agent.py`), a
...
1. **Inline, per request (`instructions`).** The agent attaches the prompt to
   ...
   (`core/agents/codex.py:102-104`). A blank prompt means the request carries no
   `instructions` at all.

2. **Synced to a container file, on save (CLI agents only).**
   `AgentConfiguration.save()` calls `_sync_agents_md()`
   (`core/models/agent.py:114-158`), which POSTs the prompt to the gateway's
   `/update-agents-md` endpoint. The gateway **overwrites** its system-prompt
   file:
   - Codex → `/app/AGENTS.md` (`docker/codex/codex_api.py:573`)
   - Claude → `/app/CLAUDE.md` (`docker/claude/claude_api.py:452`)
...
The Claude gateway's `_resolve_instructions` (`docker/claude/claude_api.py:174-180`)
does:
...
`console/templates/console/agents/form.html`). Saving runs the normal
...
(`console/views/agent.py` · `_default_system_prompt_path`):

- `claude_code` / `claude_code_api` → seeds from `docker/claude/CLAUDE.md`
- every other CLI / Direct API type → seeds from `docker/codex/AGENTS.md`
...
> (`COPY docker/codex/AGENTS.md` and `COPY docker/claude/CLAUDE.md`). Without
> them the textarea would render empty on a blank-prompt agent. The view resolves
> them at `/app/docker/codex/AGENTS.md` and `/app/docker/claude/CLAUDE.md` inside
> the container.
...
- `docker/codex/AGENTS.md` (Codex)
- `docker/claude/CLAUDE.md` (Claude Code)
...
| Concern | Location |
|---|---|
| Prompt column (source of truth) | `core/models/agent.py` · `AgentConfiguration.system_prompt` |
| Sync to container on save | `core/models/agent.py:114-158` · `_sync_agents_md()` |
| Direct API early-return (no file) | `core/models/agent.py:120-123` |
| Inline `instructions` (Codex/Claude) | `core/agents/codex.py:102-104` |
| Direct API system message | `core/agents/direct_api.py:286-287` |
| Codex: write file | `docker/codex/codex_api.py:573` · `/update-agents-md` |
| Codex: prepend to PROMPT | `docker/codex/codex_api.py:356-359` |
| Claude: write file | `docker/claude/claude_api.py:454-457` · `/update-agents-md` |
| Claude: per-request fallback | `docker/claude/claude_api.py:174-180` · `_resolve_instructions` |
| Claude: `--append-system-prompt` | `docker/claude/claude_api.py:247-249` |
| Edit form pre-fill | `console/views/agent.py` · `_default_system_prompt_path` |
| System Prompt card (UI) | `console/templates/console/agents/form.html` |
| Default prompts copied into web image | `Dockerfile` · `COPY docker/.../AGENTS.md` `CLAUDE.md` |
