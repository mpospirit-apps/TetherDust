...
flowchart TD
    ...
    subgraph web["Django + Channels · web:8000"]
        ...
        Agent["OllamaAgent ⊂ OpenAICompatibleAgent<br/>engine/agents/direct_api.py<br/>in-process tool loop — identical to Direct API"]
    end
...
`OllamaAgent` subclass (`engine/agents/direct_api.py`):
...
type in `engine/agents/__init__.py` and `AgentConfiguration.AGENT_TYPE_CHOICES`;
...
In the management: **Agents → Add Agent → "Local LLM with Ollama"**, then:
