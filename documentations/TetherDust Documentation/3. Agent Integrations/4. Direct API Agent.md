...
flowchart TD
    ...
    subgraph web["Django + Channels · web:8000"]
        ...
        Agent["OpenAICompatibleAgent · core/agents/direct_api.py<br/><br/>in-process tool loop (httpx.AsyncClient):<br/>register_filter → tokenized mcp_url<br/>open MCP ClientSession<br/>while not done: POST /chat/completions<br/>tool_call → session.call_tool · feed result · repeat"]
    end
...
| Layer | File | Role |
|---|---|---|
| Agent abstraction | `core/agents/base.py` — `BaseAgent` | Transport-agnostic `chat()` async-generator contract. |
| Agent | `core/agents/direct_api.py` — `OpenAICompatibleAgent` | Owns the in-process tool loop: streams the provider, executes MCP tools, re-yields framed chunks. Registered under `openai_api`, with `OpenAIPlatformAgent` / `ClaudeConsoleAgent` preset subclasses for `openai_platform` / `claude_console`. |
| MCP client | `core/agents/mcp_session.py` | `open_mcp_session` (an initialized `ClientSession` over streamable-HTTP), `mcp_tools_to_openai` (MCP schema → OpenAI function schema), `call_tool_text` (flatten a tool result). |
| Filter helper | `core/agents/mcp_filter.py` | Agent-agnostic `register_filter` / `tokenized_mcp_url` / `clear_filter` / `mcp_base_url`. Reused as-is from the CLI integrations. |
| History helper | `core/agents/history.py` | `messages_to_prompt` (used by the CLI path); this agent consumes the structured turns natively. |
| Stream protocol | `core/agents/stream.py` | NUL-prefixed marker format + `parse_chunk`. |
| Factory | `core/agents/__init__.py` — `get_agent()` | Maps `openai_api` / `openai_platform` / `claude_console` → the `OpenAICompatibleAgent` family. |
| Config model | `core/models/agent.py` — `AgentConfiguration` | Persisted agent settings (see [Configuration](#configuration)). |

There is **no gateway service** and no `docker/<name>/` directory — that whole
...
> `ClientSession` (the same SDK `docker/local_mcp/local_mcp_api.py` uses over
...
2.  The chat consumer calls get_agent() → OpenAICompatibleAgent, then chat(...)
    ...
    Inside OpenAICompatibleAgent.chat() (core/agents/direct_api.py):
...
4.  Always register a per-request MCP token:
        register_filter(...)  →  POST mcp:8001/register-filter  →  token
        mcp_url = tokenized_mcp_url(token) = http://mcp:8001/mcp/<token>
...
(`core/agents/mcp_filter.py`). The only difference is where the tokenized URL
...
flowchart TD
    ...
    Register["POST mcp:8001/register-filter<br/>MCP stores: token → restrictions<br/>(register_filter)"]
    URL["mcp_url = http://mcp:8001/mcp/token<br/>restricted or all-access token"]
...
`core/agents/stream.py` defines the flat-string contract between agents and the
...
| `MCP_BASE_URL` | `http://mcp:8001` | MCP server base URL (shared with the filter helper). |
