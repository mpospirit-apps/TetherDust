...
|---|---|---|---|---|
| **OpenAI** *(default)* | Codex CLI | ChatGPT Plus/Pro | `docker/codex` (`codex:8002`) | `auth.json` via device-code login, refreshed automatically |
| **Anthropic** | Claude Code CLI | Claude Pro/Max | `docker/claude` (`claude:8002`) | OAuth token via `claude setup-token`, stored once |
...
flowchart TD
    ...
    subgraph web["Django + Channels · web:8000"]
        ...
        Agent["CodexAgent · core/agents/codex.py"]
        Filter["mcp_filter.py · register / clear"]
    end

    subgraph codex_gw["Codex API Gateway · codex:8002 · docker/codex/codex_api.py"]
    ...
    MCP["MCP Server · mcp:8001<br/>/mcp/token → role enforcement<br/>built-in tools (query_database …)"]
...
| Agent | `core/agents/codex.py` — `CodexAgent` | POSTs to the codex gateway, parses its SSE, re-yields framed chunks. Registered under the `codex` agent type. |
| Factory | `core/agents/__init__.py` — `get_agent()` | Reads the active `AgentConfiguration` and returns the matching agent (`{"codex": CodexAgent}`). Falls back to an unconfigured `CodexAgent` if none is active. |
| Filter helper | `core/agents/mcp_filter.py` | Agent-agnostic `register_filter` / `tokenized_mcp_url` / `clear_filter`. Any future agent reuses these instead of re-implementing the handshake. |
...
| Gateway | `docker/codex/codex_api.py` | FastAPI wrapper owning a single Codex CLI invocation: credential seeding, MCP config, subprocess lifecycle, JSONL→SSE, device-code login endpoints. |
| Sync task | `core/tasks.py` — `sync_codex_auth_token` | Hourly Celery job that pulls the refreshed credential back into the DB and warns near expiry. |
...
2.  The chat consumer calls get_agent() → CodexAgent, then CodexAgent.chat(...)
    ...
    Inside CodexAgent.chat() (core/agents/codex.py):
3.  Always register a per-request MCP token:
        register_filter(...)  →  POST mcp:8001/register-filter  →  token
        payload["mcp_url"] = tokenized_mcp_url(token)  = http://mcp:8001/mcp/<token>
...
- `console/views/agent.py:agent_device_login_start` proxies `POST /auth/device/start`
...
flowchart TD
    ...
    Register["POST mcp:8001/register-filter<br/>MCP stores: token → restrictions<br/>(mcp_filter.register_filter)"]
    URL["mcp_url = http://mcp:8001/mcp/token<br/>handed to the agent"]
...
- **Registration is agent-agnostic.** It lives in Django
  (`core/agents/mcp_filter.py`), where the role data already is — not in the
...
in the console at `console/forms/agent.py` / `console/views/agent.py`:
...
**Service-URL resolution** (used by `CodexAgent`, the console device-login
...
own container (`docker/claude`, `claude:8002`), authenticated with a **Claude
...
|---|---|---|
| Gateway | `docker/codex/codex_api.py` | `docker/claude/claude_api.py` |
