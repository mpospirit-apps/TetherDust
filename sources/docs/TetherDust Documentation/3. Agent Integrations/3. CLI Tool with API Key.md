...
|---|---|---|---|
| `codex_api` | Codex CLI | `containers/codex` · `codex-api:8002` | Per-request `auth.json` file (Codex's `exec` reads credentials only from a file, never an env var). |
| `claude_code_api` | Claude Code | `containers/claude` · `claude-api:8002` | `ANTHROPIC_API_KEY` env var on the per-request subprocess (the documented headless path; `claude -p` sends it as the `X-Api-Key` header). |
...
flowchart TD
    ...
    subgraph web["Django + Channels · web:8000"]
        ...
        Agent["CodexApiAgent · engine/agents/codex.py"]
        Filter["mcp_filter.py · register / clear"]
    end

    subgraph codex_api_gw["Codex API Gateway · codex-api:8002 · containers/codex/codex_api.py"]
...
`ClaudeCodeApiAgent`, `claude-api:8002`, and `containers/claude/claude_api.py`
...
| Layer | File | Role |
|---|---|---|
| Agent (Codex) | `engine/agents/codex.py` — `CodexApiAgent` | **Subclass of `CodexAgent`.** Reuses the entire streaming + MCP-filter pipeline; overrides only credential injection and service-URL resolution. Registered under the `codex_api` agent type. |
| Agent (Claude) | `engine/agents/claude.py` — `ClaudeCodeApiAgent` | **Subclass of `ClaudeCodeAgent`** (itself a `CodexAgent`). Same overrides — credential + service-URL chain. Registered under the `claude_code_api` agent type. |
| Factory | `engine/agents/__init__.py` — `get_agent()` | Maps `codex_api` → `CodexApiAgent`, `claude_code_api` → `ClaudeCodeApiAgent`. |
| Filter helper | `engine/agents/mcp_filter.py` | Same agent-agnostic `register_filter` / `tokenized_mcp_url` / `clear_filter` used by every integration. |
| Stream protocol | `engine/agents/stream.py` | Same NUL-prefixed marker format + `parse_chunk`. |
| Config model | `engine/models/agent.py` — `AgentConfiguration` | Persisted settings; the credential lives in `_api_key` (see [Configuration](#configuration)). |
| Gateway (Codex) | `containers/codex/codex_api.py` | The **same** FastAPI wrapper as the auth-token option; for an API-key request it writes a per-request apikey `auth.json` into an isolated Codex home instead of seeding the volume subscription credential. |
| Gateway (Claude) | `containers/claude/claude_api.py` | The **same** wrapper as the OAuth-token Claude option; for an API-key request it sets `ANTHROPIC_API_KEY` on the subprocess env (clearing `CLAUDE_CODE_OAUTH_TOKEN`) instead of injecting the OAuth token. |
...
token so a stray value can't shadow it (`containers/claude/claude_api.py`):
...
2. `management/views/agent.py` encrypts it via `config.set_api_key(...)` (Fernet) and
...
codex-api:
  build:
    context: .
    dockerfile: containers/codex/Dockerfile     # same image as `codex`
  environment:
    - CODEX_API_PORT=8002
  depends_on:
    mcp:
...
claude-api:
  build:
    context: .
    dockerfile: containers/claude/Dockerfile    # same image as `claude`
  environment:
...
|---|---|---|
| Codex CLI *(implemented — `codex_api`)* | `containers/codex` | per-request apikey `auth.json` (`exec` ignores the env var) |
| Claude Code *(implemented — `claude_code_api`)* | `containers/claude` | `ANTHROPIC_API_KEY` env var on the `claude -p` subprocess |
