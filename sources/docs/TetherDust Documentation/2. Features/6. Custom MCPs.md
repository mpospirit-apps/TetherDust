...
(`containers/local_mcp/local_mcp_api.py`, port 8003). The Django web container does
...
| **The `local-mcp` container's own code** (`containers/local_mcp/`) | Rebuild: `docker compose up -d --build local-mcp`. |
