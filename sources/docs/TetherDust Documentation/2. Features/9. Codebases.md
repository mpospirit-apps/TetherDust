...
The Django side (management + Celery) **writes** configuration and caches the tree;
the MCP side **reads** the repository live for the agent. The two never share a
clone — they share the GitHub API and the `engine_codebase` table.

---

## The Codebase model

A **`Codebase`** (`web/engine/models/connections.py`) is the database
...
`parse_owner_repo()` (in the Django-free `engine/integrations/github_client.py`)
...
Staff manage codebases at **Console → Codebases** (`management/views/codebase.py`),
which mirrors the Databases tab:

- **List** (`/management/codebases/`) — every codebase with its repository, branch,
...
disk. Instead `sync_codebase` (`engine/tasks.py`, a Celery task) refreshes the
...
4. Apply the codebase's `subpath`, `include_globs`, and `exclude_globs`
   (`filter_tree()` in `engine/integrations/github_client.py`) — directories are
...
straight from the app database via `ADMIN_DATABASE_URL` (`engine_codebase`) and
...
- **Documentation generation** — the AI doc and doc-library generators
  (`management/views/docsource.py`) offer codebase checkboxes; selected codebases
  are passed to the agent so generated docs are grounded in real source.
- **Dashboard generation** — the dashboard generator
  (`management/views/dashboard.py`) likewise offers codebases as context.
...
| **Staff / management-managed admin roles** | Bypass the restriction entirely (`None` = unrestricted). |
| **Role with none granted** | Empty set = deny-all; the codebase tools return "not available for your role". |

The allow-list rides through the MCP filter token (`engine/agents/mcp_filter.py` →
