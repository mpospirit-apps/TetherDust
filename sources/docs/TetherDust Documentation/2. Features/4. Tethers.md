...
8. [Managing tethers in the management](#managing-tethers-in-the-management)
...
flowchart TD
    ...
    subgraph workspace["Portal (end user)"]
        View["workspace/views/tethers.py\nviewer + graph JSON"]
...
Two models in `engine/models/tethers.py` back the feature.
...
The management form offers active **Codebases** for the code side and
...
and validated in `engine/engines/tether_engine.py` (and re-validated in the MCP
...
Generation is triggered from the management — either automatically when a new
...
Key behaviours from `engine/engines/tether_engine.py`:
...
- **Live status** — each streamed chunk is scrubbed and written to
  `agent_log_excerpt`, so the management detail page can poll
...
The end-user viewer lives in `workspace/views/tethers.py` and is login-required:
...
Tethers are gated at two levels, both enforced in every workspace view:
...
## Managing tethers in the management

Staff manage tethers at **Console → Tethers** (`management/views/tether.py`):
