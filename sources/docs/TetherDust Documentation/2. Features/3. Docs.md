...
documentation using the active AI agent directly from the management.
...
A **DocumentationSource** (`engine/models/connections.py`) is the database record
...
| `doc_type` | Category label — used in AI prompts and the management UI. |
...
The list view at `management/docsources/` calls this automatically on every page
...
```
1. GET /docs/  →  workspace/views/docs.py:docs_view
   ...
   - Renders workspace/docs.html with the sidebar tree.
...
At render time, `WikiLinkExtension` (in `workspace/views/docs.py`) resolves the
...
**Role assignment** — in the management, open a Role's edit form and add the
...
the management at **Console → Documentation Sources → Generate with AI**
...
Built-in prompts are assembled in `engine/prompts/docs.py` from layered pieces, so
...
from `engine.prompts` and covered by `tests/test_doc_prompts.py`.
