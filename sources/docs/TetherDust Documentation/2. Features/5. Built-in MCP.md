...
The **built-in MCP server** is the set of engine tools that ship with TetherDust. It is a standalone Python service (the `mcp` container, port 8001) that exposes database-querying, documentation, dashboard, and tether tools to the active AI agent over the Model Context Protocol. Every chat answer that touches data flows through these tools. The server is always active, cannot be deleted, and its individual tools can be enabled, disabled, and granted to roles from the management.
...
9. [Managing the built-in server in the management](#managing-the-built-in-server-in-the-management)
...
flowchart TD
    subgraph web["Django (web:8000)"]
        ...
        Filter["engine/agents/mcp_filter.py\nregister_filter · tokenized_mcp_url"]
        DBmirror[("engine_mcpserverconfiguration\ncore_toolconfiguration\n(admin mirror)")]
    end
...
2. **The database mirror** — a single `MCPServerConfiguration` row with `is_builtin=True`, plus one `ToolConfiguration` row per tool. These rows do **not** run anything. They exist so the management can display and manage the server, and so roles can grant or revoke individual tools.

The agent reaches the built-in tools through the *service*, not through the mirror — so the tools work even if the mirror rows are missing. The mirror only governs **management visibility** and **role-based tool grants**.
...
The container resolves its documentation sources by querying the app database directly via `ADMIN_DATABASE_URL` (`_load_sources_from_admin_db()` reads `engine_documentationsource`), since the MCP service runs without Django. If that is unavailable it falls back to filesystem resolution.

---

## The database mirror

Two models in `web/engine/models/connections.py` back the mirror.

**`MCPServerConfiguration`** — one row per MCP server. The built-in server is the row with `is_builtin=True`. It has a blank `url`/`transport` (it is served in-process, not over a configured URL) and is always `is_active=True`. Built-in rows are protected: the management refuses to edit, delete, or connectivity-test them.
...
| `display_name` | Human-readable label shown in the management. |
...
Because the built-in rows are reference data, they are seeded idempotently rather than hand-created. The canonical definition lives in `web/engine/builtin_mcp.py` (`BUILTIN_SERVER_NAME`, `BUILTIN_TOOLS`, and `ensure_builtin_mcp()`), and it is wired to Django's `post_migrate` signal in `engine/apps.py`:
...
1. The chat consumer resolves the user's permissions from their role — `UserProfile.get_allowed_tools()` (`engine/models/auth.py`) and the matching `get_allowed_databases()`, `get_allowed_doc_sources()`, `get_allowed_codebases()`, `get_max_row_limit()`.
2. `engine/agents/mcp_filter.py` registers a per-request token with the MCP server (`register_filter()` → `POST /register-filter`) and hands the agent a tokenized URL (`tokenized_mcp_url()` → `/mcp/<token>`). This happens even for unrestricted users; an all-`None` filter is an all-access token.
...
## Managing the built-in server in the management
...
The server itself is read-only: the edit, delete, and connectivity-test actions are blocked for built-in rows (`management/views/mcp_server.py`). Custom MCP servers — added and edited freely — are covered in **6. Custom MCPs**.
...
| **Tool config in the management** (description, enable/disable, role grants) | Live — read from the database on every request. No restart. |
| **A custom (local) MCP server's config** | Live — the management pings the local-mcp `/reload` endpoint on save. |
| **A custom MCP server's code** (`containers/local_mcp/`) | Not mounted; rebuild with `docker compose up -d --build local-mcp`. |
