...
Call sites: ``console/views/docsource.py``. The static template text lives here;
...
the codebase name and file path (e.g. `core/agents/base.py`) naming the real
...
`core/agents/base.py` and `BaseAgent.chat` builds trust; documentation that asserts
...
5. Identify the two-to-five core abstractions everything revolves around; these anchor the
...
- Every subsystem page names its core data structure and tables its members
