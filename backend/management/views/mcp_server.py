...
def mcp_server_list_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        request,
        "management/mcp_servers/list.html",
        {
            "servers": servers,
            "section": "mcp_servers",
        },
...
def mcp_server_form_view(request: HttpRequest, pk: str | None = None) -> HttpResponse:
    ...
    return render(
        request,
        "management/mcp_servers/form.html",
        {
            "form": form,
            "instance": instance,
            "section": "mcp_servers",
...
def prompt_form_view(request: HttpRequest, server_pk: str, pk: str | None = None) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            prompt.save()
            return redirect("management:mcp_server_detail", pk=server.pk)
    else:
        form = PromptConfigurationForm(instance=instance)

    return render(
        request,
        "management/prompts/form.html",
        {
            "form": form,
            "instance": instance,
            "server": server,
