...
def user_create_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect("console:user_list")
    else:
        form = UserCreateForm()

    return render(
        request,
        "console/users/create.html",
        {
            "form": form,
            "section": "users",
        },
